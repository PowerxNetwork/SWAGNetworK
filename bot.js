...
client.on('message',async message => { ///By KillerFox
    ...
    var gMembers;
    var filter = m => m.author.id === message.author.id;
    if(message.content.startsWith("&bc")) { ///By KillerFox
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("559168782002814976");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("587784465196122117");
    if (!channel) {
...
client.on("message", message => {
            ...
            if (message.content.startsWith(prefix + "0502531696")) {
                         if (!message.member.hasPermission("ADMINISTRATOR"))  return;
  let args = message.content.split(" ").slice(1);
